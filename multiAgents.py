        ...
        def minValue(gameState, depth, ghostIndex):
            ...
            for action in gameState.getLegalActions(ghostIndex):
                ...
                if ghostIndex == gameState.getNumAgents() - 1:
                    min_value = min(min_value, maxValue(successor, depth - 1))
                else:
                    min_value = min(min_value, minValue(successor, depth, ghostIndex + 1))
            return min_value
        ...
        def maxValue(gameState, depth):
            ...
            for action in gameState.getLegalActions(self.index):
                if action != Directions.STOP:
                    ...
                    max_value = max(max_value, minValue(successor, depth, 1))
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        #same terminal state as earlier
        def isTerminalState(gameState, depth):
            return gameState.isWin() or gameState.isLose() or depth == 0
        ...
        def getMaxValue(gameState, depth):
            ...
            for action in gameState.getLegalActions(self.index):
                if action != Directions.STOP:
                    ...
                    max_value = max(max_value, getExpectedValue(successor, depth, 1))

            return max_value
        

        #getting the expected value for the ghosts
        def getExpectedValue(gameState, depth, ghostIndex):
            ...
            probability_value = 1.0 / len(actionList)

            if ghostIndex < gameState.getNumAgents() - 1:
                nextGhostIndex = ghostIndex + 1
            else:
                nextGhostIndex = self.index
            ...
            for action in actionList:
                ...
                if ghostIndex < gameState.getNumAgents() - 1:
                    totalValue += probability_value * getExpectedValue(successor, depth, nextGhostIndex)
                else:
                    totalValue += probability_value * getMaxValue(successor, depth - 1)

            return totalValue

        #getting the best action for Pacman after state eval
        legalActions = []
        for action in gameState.getLegalActions(self.index):
            if action != Directions.STOP:
